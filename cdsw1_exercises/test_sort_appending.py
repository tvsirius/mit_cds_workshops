...
from sort_appending import sort_appending


# from experiment, it was seen that sort_appending sort the list and return it, leaving the original list empty
# some glass box test added after formatting the function

class TestSortAppending(unittest.TestCase):
    """
    A unittest test case for the 'sort_appending' function.

    This test case covers a variety of scenarios to ensure the correctness of the 'sort_appending' function,
    ...
    def test_0(self):
        """tests empty list"""
        self.assertEqual(sort_appending([]), [])

    def test_1(self):
        """tests 1 element list"""
        self.assertEqual(sort_appending([1]), [1])

    def test_2(self):
        """tests some list"""
        self.assertEqual(sort_appending([1, 0, 10, 4, 2]), [0, 1, 2, 4, 10])

    def test_3(self):
        """tests some list"""
        self.assertEqual(sort_appending([1.0, 0.0, 10.0, 4.0, 2.0]), [0.0, 1.0, 2.0, 4.0, 10.0])

    def test_4(self):
        """tests some list"""
        self.assertEqual(sort_appending([100.0, -4, 346, 0, 7, 2, 10]), [-4, 0, 2, 7, 10, 100.0, 346])

    def test_5(self):
        """tests list with chars"""
        self.assertEqual(sort_appending(['e', 'f', 'l', 'a', 'b', 'z']), ['a', 'b', 'e', 'f', 'l', 'z'])
    ...
    def test_8(self):
        ...
        #
        #  !!!   it seems sort_appending uses recustion, so with  L=[random.randint(-10000000,-10000000) for _ in range(1000)]
    ...
    def test_9(self):
        """test with L2"""
        self.assertEqual(sort_appending([], ['e', 'f', 'l', 'a', 'b', 'z']), ['e', 'f', 'l', 'a', 'b', 'z'])

    def test_10(self):
        """test L1 sorted is appended to L2"""
        self.assertEqual(sort_appending(['e', 'f', 'l', 'a', 'b', 'z'], [1, 0, 10, 4, 2]),
